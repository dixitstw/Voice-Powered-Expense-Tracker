...
export const Provider = ({ children }) => {
  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      try {
        ...
        const response = await fetch(`http://localhost:5001/api/gettransactions`, {
          headers: {
  ...
  const deleteTransaction = async (id) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5001/api/deletetransactions/${id}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
  ...
  const addTransaction = async (transaction) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://localhost:5001/api/posttransactions`, {
        method: 'POST',
        headers: {
